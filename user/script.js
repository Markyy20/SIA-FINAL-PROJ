...
function updatePHTime() {
  ...
  let hours = currentTime.getUTCHours() % 12 || 12;
  let minutes = currentTime.getUTCMinutes().toString().padStart(2, '0');
  let seconds = currentTime.getUTCSeconds().toString().padStart(2, '0');
  let ampm = currentTime.getUTCHours() < 12 ? 'AM' : 'PM';


  document.getElementById('timeAndDate').textContent = hours + ':' + minutes + ':' + seconds + ' ' + ampm;
}
